...
(defmethod process ((tagger pos-tagger) sentence)
  ...
  (with-slots (weights unique-tag-words) tagger
    (let ((scores (make-array (length sentence) :element-type 'hash-table)))
      (iter (for prev-tag      :previous  tag      :initially  'start)
            (for prev-prev-tag :previous  prev-tag :initially  'start)
            (for word          :in-vector sentence :with-index i)
            (for form          :next      (word-form word))
            (for unique-tag    :next      (gethash form unique-tag-words))
            (for tag           :next      unique-tag)
            (setf (svref scores i) (make-hash-table :test #'eq))
            (if unique-tag
                (setf (gethash unique-tag (svref scores i)) most-positive-fixnum)
              (progn
                (iter (for feature :in-vector (pos-backward-features word sentence prev-tag prev-prev-tag))
                ...
                (setq tag (iter (for (class weight) :in-hashtable (svref scores i))
                              (finding class :maximizing weight))))))
      (iter (for next-tag      :previous  tag      :initially  'end)
            (for next-next-tag :previous  next-tag :initially  'end)
            (for word          :in-vector sentence :with-index i   :downto 0)
            (for form          :next      (word-form word))
            (for unique-tag    :next      (gethash form unique-tag-words))
            (for tag           :next       unique-tag)
            (unless unique-tag
              (iter (for feature :in-vector (pos-forward-features word sentence next-tag next-next-tag))
                    (when-let (table (apply #'href-default nil weights feature))
                      (iter (for (class weight) :in-hashtable table)
                            (incf (gethash class (svref scores i) 0.0) weight))))
              (setq tag (iter (for (class weight) :in-hashtable (svref scores i))
                              (finding class :maximizing weight)))))
      (iter (for table :in-vector scores)
            (collect (iter (for (class weight) :in-hashtable table)
                           (finding class :maximizing weight)))))))
  ...
  (with-slots (unique-tag-words) tagger
    ...
    (iter (for cycle :range cycles)
          (let ((correct-count    0)
            ...
            (iter (for sentence :in-vector sentences)
                  (for guesses :next (process tagger sentence))
                  (iter (for prev-tag      :previous  guess    :initially  'start)
                        (for prev-prev-tag :previous  prev-tag :initially  'start)
                        (for word          :in-vector sentence :with-index i)
                        (for guess         :in        guesses)
                        (for truth         :next      (word-upos word))
                        (for features      :next      (pos-all-features word sentence prev-tag prev-prev-tag
...
(defmethod test ((tagger pos-tagger) sentences)
  (let ((correct-count       0)
    ...
    (iter (for sentence :in-vector sentences)
          (iter (for word  :in-vector sentence)
                (for guess :in        (process tagger sentence))
                (for truth :next      (word-upos word))
                (if (eq guess truth)
                    #|(or (eq guess truth)
